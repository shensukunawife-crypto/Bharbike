...
export async function subscriptionBrainPage(req, res) {
  try {
    const [brainLogsRes, brainSweepRes] = await Promise.all([
    ...
    return renderPage(res, {
      title: "Subscription Brain",
      active: "subscription-brain",
      bodyView: "subscription-brain",
      stats,
    });
  } catch (error) {
    console.error("[admin.subscriptionBrainPage] failed", error);
    return res.status(500).send("Unable to load Subscription Brain page");
  }
}
